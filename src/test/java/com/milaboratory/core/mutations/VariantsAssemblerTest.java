package com.milaboratory.core.mutations;

import com.milaboratory.core.Range;
import com.milaboratory.core.alignment.Alignment;
import com.milaboratory.core.mutations.generator.NucleotideMutationModel;
import com.milaboratory.core.sequence.NucleotideSequence;
import com.milaboratory.test.TestUtil;
import com.milaboratory.util.IntArrayList;
import com.milaboratory.util.RandomUtil;
import org.apache.commons.math3.random.Well19937c;
import org.junit.Test;

import java.util.Arrays;

import static com.milaboratory.core.mutations.Mutations.EMPTY_NUCLEOTIDE_MUTATIONS;
import static com.milaboratory.core.mutations.generator.MutationModels.getEmpiricalNucleotideMutationModelLowIndels;
import static com.milaboratory.core.mutations.generator.MutationsGenerator.generateMutations;

/**
 * @author Dmitry Bolotin
 * @author Stanislav Poslavsky
 */
public class VariantsAssemblerTest {
    public static Allele[] generateAlleles(NucleotideSequence reference,
                                           int numberOfAlleles,
                                           NucleotideMutationModel model,
                                           boolean includeWt,
                                           boolean recurrent) {
        Allele wt = new Allele(includeWt ? numberOfAlleles - 1 : -1, null, EMPTY_NUCLEOTIDE_MUTATIONS, EMPTY_NUCLEOTIDE_MUTATIONS, reference);

        Well19937c random = RandomUtil.getThreadLocalRandom();
        Allele[] alleles = new Allele[numberOfAlleles];
        for (int i = 0; i < (includeWt ? numberOfAlleles - 1 : numberOfAlleles); i++) {
            int ref = -1;
            if (recurrent && i > 0)
                ref = random.nextInt(i);
            Allele parent = ref == -1 ? wt : alleles[ref];
            Mutations<NucleotideSequence> rMutations = generateMutations(parent.getMutated(), model);
            while (rMutations.isEmpty())
                rMutations = generateMutations(parent.getMutated(), model);
            Mutations<NucleotideSequence> tMutations = parent.totalMutations.combineWith(rMutations);
            alleles[i] = new Allele(i, parent, rMutations, tMutations, reference);
        }
        if (includeWt)
            alleles[alleles.length - 1] = wt;

        return alleles;
    }

    public static Clone[] generateClones(Allele[] alleles,
                                         int numberOfClones, float randomCoverageOffset,
                                         int minNumberOfReads, int maxNumberOfReads,
                                         NucleotideMutationModel hypermutationsModel,
                                         NucleotideMutationModel noiseModel) {
        Well19937c random = RandomUtil.getThreadLocalRandom();

        Clone[] clones = new Clone[numberOfClones];
        for (int i = 0; i < numberOfClones; i++) {
            Allele allele = alleles[random.nextInt(alleles.length)];
            MutationConsensusBuilder<NucleotideSequence> builder =
                    new MutationConsensusBuilder<>(NucleotideSequence.ALPHABET, allele.reference.size());

            final NucleotideSequence alleleSequence = allele.getMutated();
            Mutations<NucleotideSequence> hypermutations = generateMutations(alleleSequence, hypermutationsModel);
            NucleotideSequence hypermutated = hypermutations.mutate(alleleSequence);
            Mutations<NucleotideSequence> alleleAndHypermutations = allele.totalMutations.combineWith(hypermutations);

            int readsCount = minNumberOfReads + random.nextInt(maxNumberOfReads - minNumberOfReads);
            for (int j = 0; j < readsCount; j++) {
                Mutations<NucleotideSequence> noiseMuts = generateMutations(hypermutated, noiseModel);
                Mutations<NucleotideSequence> totalMutations = alleleAndHypermutations.combineWith(noiseMuts);

                final int randomCoverageOffsetSize = (int) (randomCoverageOffset * allele.reference.size());
                Range r = new Range(randomCoverageOffsetSize == 0 ? 0 : random.nextInt(randomCoverageOffsetSize),
                        allele.reference.size() - (randomCoverageOffsetSize == 0 ? 0 : random.nextInt(randomCoverageOffsetSize)));
                totalMutations = totalMutations.extractMutationsForRange(r).move(r.getFrom());

                Alignment<NucleotideSequence> alignment = new Alignment<>(allele.reference, totalMutations, r, 0);
                builder.aggregate(alignment, Weight.ONE);
            }

            clones[i] = new Clone(allele, hypermutations, builder.build());
        }
        return clones;
    }

    public static final class Allele {
        final int id;
        final Allele referenceAllele;
        final Mutations<NucleotideSequence> totalMutations;
        final Mutations<NucleotideSequence> relativeMutations;
        final NucleotideSequence reference;

        public Allele(int id, Allele referenceAllele,
                      Mutations<NucleotideSequence> relativeMutations,
                      Mutations<NucleotideSequence> totalMutations,
                      NucleotideSequence reference) {
            this.id = id;
            this.referenceAllele = referenceAllele;
            this.totalMutations = totalMutations;
            this.relativeMutations = relativeMutations;
            this.reference = reference;
        }

        NucleotideSequence getMutated() {
            return totalMutations.mutate(reference);
        }

        @Override
        public String toString() {
            return "Allele{" +
                    "\n\tid                = " + id +
                    "\n\treferenceAllele   = " + (referenceAllele == null ? -1 : referenceAllele.id) +
                    "\n\trelativeMutations = " + relativeMutations +
                    "\n\ttotalMutations    = " + totalMutations +
                    "\n}";
        }
    }

    public static final class Clone {
        final Allele allele;
        final Mutations<NucleotideSequence> hypermutations;
        final AggregatedMutations<NucleotideSequence> aggregator;

        public Clone(Allele allele, Mutations<NucleotideSequence> hypermutations,
                     AggregatedMutations<NucleotideSequence> aggregator) {
            this.allele = allele;
            this.hypermutations = hypermutations;
            this.aggregator = aggregator;
        }
    }

    static AggregatedMutations<NucleotideSequence>[] getAggregators(Clone[] clones) {
        AggregatedMutations[] aggregators = new AggregatedMutations[clones.length];
        for (int i = 0; i < clones.length; i++)
            aggregators[i] = clones[i].aggregator;
        return aggregators;
    }

    @Test
    public void test1() throws Exception {
        final int minReadsLength = 100;
        final int maxReadsLength = 200;
        final int minReadsNumber = 1;
        final int maxReadsNumber = 30;
        final int numberOfClones = 15_000;
        final boolean includeWt = true;
        final boolean doRecursionAlleleCreation = true;

        final int numberOfAlleles = 6;
        final float randomCoverageOffset = 0.2f;
        final int alleleRate = 5;
        final int hypermutationsRate = 10;
        final int noiseRate = 10;

        final VariantsAssemblerParameters assemblerParameters = new VariantsAssemblerParameters(
                1000, 1000,
                new AggregatedMutations.SimpleMutationsFilter(1, 0.7),
                0.9f,
                10f, 1f,
                Integer.MAX_VALUE);

        Well19937c random = RandomUtil.getThreadLocalRandom();
        random.setSeed(213);

        NucleotideMutationModel iModel = getEmpiricalNucleotideMutationModelLowIndels();
        NucleotideMutationModel
                alleleModel = iModel.multiplyProbabilities(alleleRate),
                hypermutations = iModel.multiplyProbabilities(hypermutationsRate),
                noise = iModel.multiplyProbabilities(noiseRate);

        alleleModel.reseed(random.nextLong());
        hypermutations.reseed(random.nextLong());
        noise.reseed(random.nextLong());

        NucleotideSequence reference = TestUtil.randomSequence(NucleotideSequence.ALPHABET, minReadsLength, maxReadsLength);
        final Allele[] alleles = generateAlleles(reference, numberOfAlleles, alleleModel, includeWt, doRecursionAlleleCreation);
        for (Allele allele : alleles)
            System.out.println(allele);
        System.out.println();

        final Clone[] clones = generateClones(alleles, numberOfClones,
                randomCoverageOffset, minReadsNumber, maxReadsNumber,
                hypermutations, noise);
        final int[] realCounts = new int[alleles.length];
        for (Clone clone : clones)
            ++realCounts[clone.allele.id];

        VariantsAssembler<NucleotideSequence> assembler = new VariantsAssembler<>(
                NucleotideSequence.ALPHABET, getAggregators(clones), assemblerParameters);

        final AssignedVariants<NucleotideSequence> variants = assembler.findVariants();
        int unassigned = 0;
        int match = 0;
        int mismatch = 0;
        int maxLevelAssignment = 0;
        for (int i = 0; i < variants.assignments.length; i++) {
            if (variants.assignments[i] == null) {
                ++unassigned;
            } else {
                maxLevelAssignment = Math.max(variants.assignments[i].depth, maxLevelAssignment);
                if (variants.alleles[variants.assignments[i].alleleId].equals(clones[i].allele.totalMutations))
                    ++match;
                else
                    ++mismatch;
            }
        }

        System.out.println("     Match: " + match);
        System.out.println("  Mismatch: " + mismatch);
        System.out.println("Unassigned: " + unassigned);
        System.out.println(" Max level: " + maxLevelAssignment);

        System.out.println();

        final IntArrayList[] mapping = mapping(alleles, variants.alleles);
        System.out.println("           Real counts: " + Arrays.toString(realCounts));
        System.out.println(" Candidates   (mapped): " + Arrays.toString(applyMapping(alleles.length, variants.counts, mapping)));
        System.out.println(" Candidates (original): " + Arrays.toString(variants.counts));
        System.out.println(" Candidates   mappings: " + Arrays.deepToString(mapping));
    }

    public static int[] applyMapping(int length, int[] array, IntArrayList[] mapping) {
        int[] newArray = new int[length];
        Arrays.fill(newArray, -1);
        for (int i = 0; i < array.length; i++) {
            if (!mapping[i].isEmpty())
                newArray[mapping[i].get(0)] = array[i];
        }
        return newArray;
    }

    public static IntArrayList[] mapping(Allele[] knownAlleles, Mutations<NucleotideSequence>[] candidates) {
        Mutations<NucleotideSequence>[] ks = new Mutations[knownAlleles.length];
        for (int i = 0; i < knownAlleles.length; i++)
            ks[i] = knownAlleles[i].totalMutations;
        return mapping(ks, candidates);
    }

    public static IntArrayList[] mapping(Mutations<NucleotideSequence>[] knownAlleles, Mutations<NucleotideSequence>[] candidates) {
        IntArrayList[] mapping = new IntArrayList[candidates.length];
        for (int i = 0; i < candidates.length; i++) {
            if (mapping[i] == null)
                mapping[i] = new IntArrayList();
            for (int j = 0; j < knownAlleles.length; j++) {
                if (knownAlleles[j].equals(candidates[i])) {
                    mapping[i].add(j);
                    break;
                }
            }

            if (mapping[i].isEmpty())
                for (int j = 0; j < knownAlleles.length; j++)
                    if (includes(knownAlleles[j], candidates[i]))
                        mapping[i].add(j);
        }
        return mapping;
    }

    public static boolean includes(Mutations<NucleotideSequence> ms, Mutations<NucleotideSequence> part) {
        final int[] rawMutations = ms.getRAWMutations();
        final int[] partRaw = part.getRAWMutations();
        out:
        for (int r : partRaw) {
            for (int mutation : rawMutations)
                if (mutation == r)
                    continue out;
            return false;
        }
        return true;
    }

//
//    @Test
//    public void test1() throws Exception {
//        final int numberOfCLones = 400;
//        final int minReadsNumber = 100;
//        final int maxReadsNumber = 1000;
//        final int minReadsLength = 100;
//        final int maxReadsLength = 200;
//        final double randomCoverageOffset = 0.2;
//        final int numberOfAlleles = 4;
//        final boolean includeWt = true;
//
//        System.out.println(RandomUtil.reseedThreadLocalFromTime());
//        //RandomUtil.reseedThreadLocal(1234);
//
//        Well19937c random = RandomUtil.getThreadLocalRandom();
//
//        NucleotideMutationModel model = getEmpiricalNucleotideMutationModel().multiplyProbabilities(30);
//        model.reseed(random.nextLong());
//
//        NucleotideMutationModel hypermutations = getEmpiricalNucleotideMutationModel()
//                .multiplyProbabilities(3);
//        hypermutations.reseed(random.nextLong());
//
//        NucleotideMutationModel noise = getEmpiricalNucleotideMutationModel().multiplyProbabilities(3);
//        noise.reseed(random.nextLong());
//
//        NucleotideSequence reference = TestUtil.randomSequence(NucleotideSequence.ALPHABET, minReadsLength, maxReadsLength);
//
//        Mutations<NucleotideSequence>[] alleles = new Mutations[numberOfAlleles];
//        NucleotideSequence[] alleleSeqs = new NucleotideSequence[numberOfAlleles];
//        for (int i = includeWt ? 1 : 0; i < numberOfAlleles; i++) {
//            alleles[i] = MutationsGenerator.generateMutations(reference, model);
//            alleleSeqs[i] = alleles[i].mutate(reference);
//        }
//        if (includeWt) {
//            alleles[0] = Mutations.EMPTY_NUCLEOTIDE_MUTATIONS;
//            alleleSeqs[0] = reference;
//        }
//
//        List<AggregatedMutations<NucleotideSequence>> aggregators = new ArrayList<>();
//        for (int i = 0; i < numberOfCLones; i++) {
//            MutationConsensusBuilder<NucleotideSequence> builder =
//                    new MutationConsensusBuilder<>(NucleotideSequence.ALPHABET, reference.size());
//
//            int alleleId = random.nextInt(numberOfAlleles);
//            Mutations<NucleotideSequence> allele = alleles[alleleId];
//            NucleotideSequence alleleSeq = alleleSeqs[alleleId];
//
//            int readsCount = minReadsLength + random.nextInt(maxReadsNumber - minReadsLength);
//            for (int j = 0; j < readsCount; j++) {
//                Mutations<NucleotideSequence> noiseMuts =
//                        MutationsGenerator.generateMutations(alleleSeq, noise);
//                Mutations<NucleotideSequence> totalMutations = allele.combineWith(noiseMuts);
//                Range r = new Range(random.nextInt(randomCoverageOffsetSize), reference.size() - random.nextInt(randomCoverageOffsetSize));
//                totalMutations = totalMutations.extractMutationsForRange(r).move(r.getFrom());
//                Alignment<NucleotideSequence> alignment = new Alignment<>(reference, totalMutations, r, 0);
//                builder.aggregate(alignment, Weight.ONE);
//            }
//
//            aggregators.add(builder.build());
//        }
//
//        VariantsAssembler<NucleotideSequence> assembler = new VariantsAssembler<>(NucleotideSequence.ALPHABET,
//                aggregators, new VariantsAssemblerParameters(50, 30, new AggregatedMutations.SimpleMutationsFilter(1, 0.7),
//                0.9f, 10f, 1f, 2));
//
//        //Arrays.sort(alleles, M_COMPARATOR);
//        final AssignedVariants<NucleotideSequence> variants = assembler.findVariants();
//        for (int i = 0; i < variants.alleles.length; i++)
//            System.out.println(i + ":  " + variants.alleles[i]);
//
//        for (VariantsAssembler.AlleleAssignmentResult assignment : variants.assignments) {
//            System.out.println(assignment);
//        }
//        Mutations<NucleotideSequence>[] actualAlleles = variants.alleles;
//        //Collections.sort(actualAlleles, M_COMPARATOR);
//        HashSet<Mutations<NucleotideSequence>> expectedSet = new HashSet<>(Arrays.asList(alleles));
//
//        System.out.println("\nUnique actual:");
//        for (Mutations<NucleotideSequence> actualAllele : actualAlleles)
//            if (!expectedSet.remove(actualAllele))
//                System.out.println(actualAllele);
//        System.out.println("\nUnique expected:");
//        for (Mutations<NucleotideSequence> expected : expectedSet)
//            System.out.println(expected);
//    }
//
//    @Test
//    public void test2() throws Exception {
////        not sorted
////        0 (46):  [S10:C->T,I13:C,S30:A->G,D47:T,D53:A,D55:C,S61:T->G,S62:A->T,D63:A,D64:C,I76:A,S80:G->T,I83:G,S100:A->G,D104:C,D107:C,D109:C,D120:C,I131:C,D141:G,I156:A,I169:A,D178:A,D179:C]
////        1 (115):  [S8:A->G,D18:C,S24:G->C,I41:G,D54:C,I65:G,D70:C,S74:A->T,S75:A->G,D87:T,D108:T,S118:G->A,D122:T,D129:C,D132:C,S160:C->T,I172:C,S184:C->T,S186:G->T]
////        2 (61):  []
////        3 (51):  [D18:C,S24:G->C,I41:G,D54:C,I65:G,D70:C,S74:A->T,S75:A->G,D87:T,D108:T,S118:G->A,D122:T,D129:C,D132:C,S160:C->T]
////        4 (44):  [D47:T,D53:A,D55:C,D63:A,D64:C,S80:G->T,S100:A->G,D104:C,D107:C,D109:C,D120:C,D141:G]
//        final int NUMBER_OF_CLONES = 400;
//        final int MIN_NUMBER_OF_READS = 1;
//        final int MAX_NUMBER_OF_READS = 10;
//        final int MIN_REF_LENGTH = 100;
//        final int MAX_REF_LENGTH = 200;
//        final double RANDOM_COVERAGE_OFFSET_SIZE = 0.2;
//        final int NUMBER_OF_ALLELES = 4;
//        final boolean FIRST_ALLELE_IS_WT = true;
//        final boolean LAST_ALLELE_IS_SECOND_ADDED = true;
//
//
////        RandomUtil.reseedThreadLocalFromTime();
//        RandomUtil.reseedThreadLocal(1234);
//        Well19937c random = RandomUtil.getThreadLocalRandom();
//
//        NucleotideMutationModel model = getEmpiricalNucleotideMutationModel().multiplyProbabilities(10);
//        model.reseed(random.nextLong());
//        NucleotideMutationModel noise = getEmpiricalNucleotideMutationModel().multiplyProbabilities(3);
//        noise.reseed(random.nextLong());
//
//        NucleotideSequence reference = TestUtil.randomSequence(NucleotideSequence.ALPHABET, MIN_REF_LENGTH, MAX_REF_LENGTH);
//        final int randomCoverageOffsetSize = (int) (RANDOM_COVERAGE_OFFSET_SIZE * reference.size());
//
//        Mutations<NucleotideSequence>[] alleles = new Mutations[NUMBER_OF_ALLELES];
//        NucleotideSequence[] alleleSeqs = new NucleotideSequence[NUMBER_OF_ALLELES];
//        for (int i = FIRST_ALLELE_IS_WT ? 1 : 0; i < NUMBER_OF_ALLELES; i++) {
//            alleles[i] = MutationsGenerator.generateMutations(reference, model);
//            alleleSeqs[i] = alleles[i].mutate(reference);
//        }
//        if (FIRST_ALLELE_IS_WT) {
//            alleles[0] = Mutations.EMPTY_NUCLEOTIDE_MUTATIONS;
//            alleleSeqs[0] = reference;
//        }
//        if (LAST_ALLELE_IS_SECOND_ADDED) {
//            alleles[alleles.length - 1] = alleles[alleles.length - 2]
//                    .combineWith(MutationsGenerator.generateMutations(alleleSeqs[alleles.length - 2], noise));
//            alleleSeqs[alleles.length - 1] = alleles[alleles.length - 1].mutate(reference);
//        }
//
//        List<AggregatedMutations<NucleotideSequence>> aggregators = new ArrayList<>();
//        for (int i = 0; i < NUMBER_OF_CLONES; i++) {
//            MutationConsensusBuilder<NucleotideSequence> builder =
//                    new MutationConsensusBuilder<>(NucleotideSequence.ALPHABET, reference.size());
//
//            int alleleId = random.nextInt(NUMBER_OF_ALLELES);
//            Mutations<NucleotideSequence> allele = alleles[alleleId];
//            NucleotideSequence alleleSeq = alleleSeqs[alleleId];
//
//            int readsCount = MIN_NUMBER_OF_READS + random.nextInt(MAX_NUMBER_OF_READS - MIN_NUMBER_OF_READS);
//            for (int j = 0; j < readsCount; j++) {
//                Mutations<NucleotideSequence> noiseMuts =
//                        MutationsGenerator.generateMutations(alleleSeq, noise);
//                Mutations<NucleotideSequence> totalMutations = allele.combineWith(noiseMuts);
//                Range r = new Range(random.nextInt(randomCoverageOffsetSize), reference.size() - random.nextInt(randomCoverageOffsetSize));
//                totalMutations = totalMutations.extractMutationsForRange(r).move(r.getFrom());
//                Alignment<NucleotideSequence> alignment = new Alignment<>(reference, totalMutations, r, 0);
//                builder.aggregate(alignment, Weight.ONE);
//            }
//
//            aggregators.add(builder.build());
//        }
//
//        VariantsAssembler<NucleotideSequence> assembler = new VariantsAssembler<>(NucleotideSequence.ALPHABET,
//                aggregators, new VariantsAssemblerParameters(50, 30, new AggregatedMutations.SimpleMutationsFilter(1, 0.7),
//                0.9f, 10f, 1f, 5));
//
//        //Arrays.sort(alleles, M_COMPARATOR);
//        final AssignedVariants<NucleotideSequence> variants = assembler.findVariants();
//        for (int i = 0; i < variants.alleles.length; i++)
//            System.out.println(i + " (" + variants.counts[i] + "):  " + variants.alleles[i]);
//
//        for (VariantsAssembler.AlleleAssignmentResult assignment : variants.assignments) {
//            System.out.println(assignment);
//        }
//
//        //Arrays.sort(alleles, M_COMPARATOR);
//        Mutations<NucleotideSequence>[] actualAlleles = assembler.findVariants().alleles;
//        //Collections.sort(actualAlleles, M_COMPARATOR);
//        HashSet<Mutations<NucleotideSequence>> expectedSet = new HashSet<>(Arrays.asList(alleles));
//
//        System.out.println("\nUnique actual:");
//        for (Mutations<NucleotideSequence> actualAllele : actualAlleles)
//            if (!expectedSet.remove(actualAllele))
//                System.out.println(actualAllele);
//        System.out.println("\nUnique expected:");
//        for (Mutations<NucleotideSequence> expected : expectedSet)
//            System.out.println(expected);
//
//        //[I6:A,S9:A->G,S13:C->T,D18:G,D29:A,D34:G,I36:T,I43:T,I60:C,S64:A->G,I81:C,D83:C,D90:C,S98:C->T,D107:A,D112:C,I134:C,D142:T,D153:T]
//        //[I6:A,S9:A->G,         D18:G,D29:A,D34:G,I36:T,I43:T,I60:C,S64:A->G,I81:C,D83:C,D90:C,S98:C->T,D107:A,D112:C,I134:C,       D153:T]
//    }
}
